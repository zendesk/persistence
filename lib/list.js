...
List.prototype.info = function (callback) {
  const multi = Persistence.getReaderClientDuringMigration().multi()
...
List.prototype.read = function (from, start, end, size, callback) {
  const redis = Persistence.getReaderClientDuringMigration()
...
List.prototype.push = function (e, callback) {
  ...
  this.idGen.alloc(function (err, value) {
    ...
    if (process.env.RADAR_MIGRATION_ENABLED === 'true') {
      ...
      if (list.maxLength > 0) {
        multiReplica.rpush(list.name, JSON.stringify(entry), Persistence.handler)
      }
      if (list.expiry) {
        multiReplica.expire(list.name, list.expiry, Persistence.handler)
      }

      if (list.maxLength) {
        multiReplica.ltrim(list.name, -list.maxLength, -1, Persistence.handler)
      }
      multiReplica.exec()
    }
  })
