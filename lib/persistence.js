...
Persistence.redis = function(value) {
  ...
  if(!connection.client || connection.subscriber.status !== 'ready') {
    logging.info({
      message: 'Client: Not connected to redis'
    });
  }
...
Persistence.pubsub = function(value) {
  ...
  if(!connection.subscriber || connection.subscriber.status !== 'ready') {
    logging.error({
      message: 'Pubsub: Not connected to redis'
    });
  }
...
Persistence.readHashValue = function (hash, key, callback) {
  logging.debug({
    message: `readHashValue: ${hash}, ${key}`
  });
  Persistence.redis()
    .hget(hash, key)
    .then((reply) => {
      ...
      if(replies) {
        Object.keys(replies).forEach(function(attr) {
          try {
            ...
            logging.error({
              message: `Corrupted key value in redis [${hash}][${attr}]. ${parseError.message}: ${parseError.stack}`
            });
            delete replies[attr];
          }
        });
...
Persistence.persistHash = function(hash, key, value) {
  logging.debug({
    message: `persistHash: ${hash}, ${key}, ${value}`
  });
