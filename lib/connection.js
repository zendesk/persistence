...
Connection.prototype.establish = function (ready) {
  ...
  if (this.readyListeners.length === 1) {
    ...
    if (process.env.RADAR_MIGRATION_ENABLED === 'true' && process.env.RADAR_ELASTICACHE_ENABLED === 'true') {
      this.connectReplicaMethod(this.config, tracker('subscriber ready :' + this.name, function (error, subscriber) {
        if (error) {
          throw (error instanceof Error) ? error : new Error(error)
        }
