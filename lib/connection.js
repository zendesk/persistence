...
var connectionMethods = {
  redis: function (config, callback) {
    ...
    // Crash when we encounter a connection error
    client.on('error', function (err) {
      logging.info('redis error:', err.message);
  ...
  sentinel: function (config, callback) {
    ...
    sentinel.on('error', function (err) {
      logging.info('sentinel error:', err.message);

      // Throw error doesn't work here at present (as it does for redis above)
      //throw new Error(err.message);
    });
